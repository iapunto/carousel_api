...
def create_app(plc):
    ...
    allowed_origins = os.getenv(
        "API_ALLOWED_ORIGINS", "http://localhost, http://127.0.0.1, http://192.168.1.0/24, http://localhost:8085,http://localhost:5001,http://127.0.0.1:8085,http://127.0.0.1:5001").split(",")
        ...
        try:
            if plc.connect():
                try:
                    ...
                    if status_code == 0 and response.get('position', -1) == 0:
                        return jsonify({'position': initial_position, 'target_position': argument, 'status_code': status_code, 'status_message': 'Advertencia: El PLC respondió con valores nulos o inválidos. Verifique la operación física.'}), 200
                    # Si todo fue bien, solo posición inicial, destino y status_code
                    return jsonify({'position': initial_position, 'target_position': argument, 'status_code': status_code, 'status_message': 'Comando enviado exitosamente al PLC.'}), 200
